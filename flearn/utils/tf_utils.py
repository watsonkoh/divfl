...
def flatten2list(object):
    ...
    for item in object:
        if isinstance(item, (list, tuple, set)):
            gather = np.append(gather, flatten2list(item))            
        else:
            gather = np.append(gather, item)
    return gather
...
def process_grad(grads):
    ...
    # return np.asarray(functools.reduce(operator.iconcat, grads, []))

    # print(grads[0],grads[1])
    client_grads = np.asarray(flatten2list(grads))
    # print('client_grads shape', len(grads), client_grads.shape)
    return client_grads
